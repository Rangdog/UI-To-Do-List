    ...
    <div class="space-y-4">
      <div v-for="step in steps" :key="step.id"
        ...
        <div class="mt-4 flex justify-between items-center">
          ...
          <router-link :to="`/steps/${step.id}`" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">
            View Details →
          </router-link>
        </div>
        <!-- COMMENT SECTION -->
        <div class="mt-4">
          <button @click="toggleComments(step.id)" class="text-sm text-indigo-600 hover:underline mb-2">
          ...
          <div v-if="expandedSteps[step.id]" class="space-y-2">
                ...
                <div v-else class="space-y-1">
                  ...
                  <div class="flex space-x-2 mt-1">
                    <button @click="saveCommentEdit(comment.id, step.id)"
                      class="text-sm text-white bg-green-600 px-3 py-1 rounded hover:bg-green-700">Save</button>
                    <button @click="cancelEdit"
                      class="text-sm text-gray-600 px-3 py-1 border rounded hover:bg-gray-200">Cancel</button>
                  </div>
                </div>
              </li>
            </ul>
            ...
            <div class="flex items-center space-x-2 mt-2">
              <input v-model="newComment[step.id]" type="text" placeholder="Nhập bình luận..."
