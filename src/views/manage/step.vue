<template>
  <div class="p-6">
    <h2 class="text-2xl font-semibold mb-4">Step Members</h2>
      ...
      <div v-for="user in userStepRole" :key="user.id" class="p-4 border rounded-xl shadow-sm">
        <div class="flex items-center justify-between gap-4 p-4 bg-white rounded-lg shadow">
          ...
          <div class="flex items-center gap-2">
            ...
            <button @click="removeOfStepMember(user.email, stepId)"
...
onMounted(async () => {
  ...
  await userRoleStore.fetchUsersRoleStep(stepId)
...
const updateUserRole = async (user: any) => {
  ...
  const res = await userRoleStore.updateUserRole({ userId: user.user_id, stepId: stepId, Slug: roleTitle || "" })
  if (res?.status) {
...
const removeOfStepMember = async (email: string, stepId: number) => {
  const res = await userRoleStore.remove(email, undefined, undefined, stepId)
